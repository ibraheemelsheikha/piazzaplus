...
# Score & output (sum of chunk similarities)
post_scores = {}
for d, dist in results:
    ...
    # sum similarities for each chunk
    post_scores[pid]['score'] += sim
# sort by total summed similarity (highest first)
sorted_posts = sorted(
    post_scores.items(),
    key=lambda x: x[1]['score'],
    reverse=True
)
print("Retrieval complete. Top 10 posts based on summed similarities:")
for idx, (pid, info) in enumerate(sorted_posts[:10], start=1):
    print(f"{idx}. Post #{pid} — {info['subject']} (total score: {info['score']:.4f})")
